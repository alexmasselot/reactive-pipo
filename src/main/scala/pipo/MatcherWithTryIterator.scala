...
/**
 *  Almost the same as EithTry, excpet that the artificial toList in findAll (to kill the iterator) is removed...
 ...
 *
 */
class MatcherWithTryIterator(val targets: List[Int], val minScore: Double, val source: DataContainer, val sliceWidth: Int) extends MatcherCommon {
  /**
